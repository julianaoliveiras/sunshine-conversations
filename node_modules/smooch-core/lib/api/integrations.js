...
var _integrationMessageTemplates = require('./integrationMessageTemplates');

var _integrationProfile = require('./integrationProfile');
...
IntegrationType.prototype.validate = function (props) {
    var missing = this.required.filter(function (field) {
        if (Array.isArray(field)) {
            return !field.some(function (subfield) {
                return props[subfield];
            });
        }
...
var integrations = {
    ...
    messenger: new IntegrationType(['pageAccessToken', 'appId', 'appSecret'], ['displayName']),
    twilio: new IntegrationType(['accountSid', 'authToken', 'phoneNumberSid'], ['displayName']),
    messagebird: new IntegrationType(['accessKey', 'originator'], ['displayName', 'signingKey']),
    telegram: new IntegrationType(['token'], ['displayName']),
    line: new IntegrationType([], ['channelAccessToken', 'channelId', 'channelSecret', 'displayName', 'qrCodeUrl']),
    viber: new IntegrationType(['token'], ['displayName']),
    wechat: new IntegrationType(['appId', 'appSecret'], ['displayName', 'encodingAesKey', 'accessTokenUrl']),
    whatsapp: new IntegrationType([], // there is two ways to to create an integration with different mandatory parameters, so everything is optional and we let the server do its job
    ['deploymentId', 'baseUrl', 'username', 'password', 'displayName', 'hsmFallbackLanguage']),
    twitter: new IntegrationType(['consumerKey', 'consumerSecret', 'accessTokenKey', 'accessTokenSecret'], ['displayName', 'tier', 'envName']),
    mailgun: new IntegrationType(['apiKey', 'domain', 'incomingAddress'], ['displayName', 'hideUnsubscribeLink', 'fromAddress']),
    fcm: new IntegrationType(['serverKey', 'senderId']),
    apn: new IntegrationType(['certificate'], [{
        name: 'autoUpdateBadge',
        type: 'boolean'
    }, 'password']),
    apple: new IntegrationType(['businessId', 'apiSecret', 'mspId'], ['displayName'])
};
...
var IntegrationsApi = exports.IntegrationsApi = function (_BaseApi) {
    ...
    function IntegrationsApi() {
        ...
        _this.profile = new (Function.prototype.bind.apply(_integrationProfile.IntegrationProfileApi, [null].concat(Array.prototype.slice.call(arguments))))();
        _this.messageTemplates = new (Function.prototype.bind.apply(_integrationMessageTemplates.IntegrationMessageTemplatesApi, [null].concat(Array.prototype.slice.call(arguments))))();
        return _this;
    }
...
Object.assign(IntegrationsApi.prototype, {
     ...
     * @memberof IntegrationsApi.prototype
     * @method list
     * @param  {(string|string[])=} types - Filter results to specific integration types
     * @param  {number} limit
     * @param  {number} offset
     * @return {APIResponse}
     */
    list: (0, _smoochMethod2.default)({
        ...
        path: '/integrations',
        func: function list(url, types, limit, offset) {
            var query = {};
